...
- Use `protectedProcedure` for all routes that require authentication
- `ctx.db` for database, `ctx.files` for S3 storage, `ctx.session` for current user
...
<template>
  ...
  <AsyncData fetch-key="files" :handler="() => $trpc.files.list.query()">
    <template #default="{ data, status, refresh }">
      ...
      <TableFiles v-else :items="data || []" @refresh-table="refresh" />
    </template>
  </AsyncData>

  <!-- Dynamic key (re-fetches when userId changes) -->
  <AsyncData :fetch-key="() => `user-${userId}`" :handler="() => $trpc.users.get.query({ id: userId })">
